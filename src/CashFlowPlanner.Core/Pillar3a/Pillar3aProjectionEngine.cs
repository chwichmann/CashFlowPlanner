...
public sealed class Pillar3aProjectionEngine
{
        ...
        DateOnly today)
    {
        ...
        foreach (var contract in plan.Pillar3aContracts.Where(x => x.IsActive))
        {
            ...
            var person = plan.Persons
                .SingleOrDefault(x => x.Id == contract.OwnerPersonId);
        ...
        DateOnly endDate)
    {
        ...
        for (var currentMonth = StartOfMonth(startDate);
        ...
        DateOnly endDate)
    {
        ...
        foreach (var contributionSchedule in contract.ContributionSchedules.Where(x => x.IsActive))
        {
            ...
            var occurrences = _scheduleOccurrenceGenerator.GenerateOccurrences(
                contributionSchedule.ToSchedule(),
                startDate,
                endDate);
        ...
        out bool closesContract)
    {
        ...
        foreach (var withdrawal in withdrawals.Where(x =>
                     x.Date >= monthStart &&
                     x.Date <= monthEnd))
        {
    ...
    private static DateOnly StartOfMonth(DateOnly date)
    {
